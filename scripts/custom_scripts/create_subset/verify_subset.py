...
def verify_subset(repo_id: str):
    ...
    if not dataset_path.exists():
        ...
        print(f"\n💡 提示:")
        print(f"   1. 运行 'python create_subset_hf_cache.py --list' 查看所有数据集")
        print(f"   2. 确保已创建该子数据集")
        return False
    ...
    if subset_meta_path.exists():
        ...
        print(f"\n   🎯 采样信息:")
        print(f"   源数据集: {subset_meta.get('source_repo_id', 'N/A')}")
        print(f"   采样比例: {subset_meta.get('fraction', 0)*100:.1f}%")
        print(f"   随机种子: {subset_meta.get('seed', 'N/A')}")
        print(f"\n   📈 统计:")
        ...
        print(f"   实际比例: {subset_meta.get('actual_fraction', 0)*100:.2f}%")
        ...
        print(f"   ⚠️  未找到 subset_metadata.json")
    ...
    try:
        ...
        try:
            ...
            print(f"   ⏳ 正在加载...", end=" ", flush=True)
            dataset = LeRobotDataset(repo_id, root=HF_LEROBOT_HOME)
            print(f"✅")
        finally:
            # 恢复环境变量
        ...
        # 获取底层的 HuggingFace Dataset
        hf_dataset = dataset.hf_dataset
        print(f"\n   📋 数据字段: {hf_dataset.column_names}")
        
        # 统计 episodes
        episodes = sorted(set(hf_dataset['episode_index']))
        print(f"\n   📈 Episode 信息:")
        print(f"   唯一 episodes: {len(episodes)}")
        print(f"   Episode ID 范围: {min(episodes)} - {max(episodes)}")
        
        # 每个 episode 的帧数
        episode_counts = Counter(hf_dataset['episode_index'])
        frames_per_ep = list(episode_counts.values())
        print(f"   每个 episode 平均帧数: {sum(frames_per_ep)/len(frames_per_ep):.1f}")
        print(f"   帧数范围: {min(frames_per_ep)} - {max(frames_per_ep)}")
        
        # 任务分布
        if 'task' in hf_dataset.column_names:
            tasks = [sample['task'] for sample in hf_dataset.select(range(min(1000, len(hf_dataset))))]
            task_counts = Counter(tasks)
            print(f"\n   📝 任务分布 (前 10 个任务，基于前 1000 样本):")
        ...
        print(f"\n💡 故障排查:")
        print(f"   1. 检查 meta/info.json 是否存在: {dataset_path}/meta/info.json")
        print(f"   2. 检查网络连接（如果需要）")
        print(f"   3. 确保数据集结构完整")
        return False
    ...
    if len(datasets_info) >= 2:
        ...
        for i in range(len(datasets_info)):
            for j in range(i+1, len(datasets_info)):
                ...
                print(f"\n   {name1} vs {name2}:")
                print(f"   重叠 episodes: {len(overlap)} ({overlap_pct:.1f}%)")
                if len(overlap) > 0 and len(overlap) <= 20:
                    print(f"   重叠的 episode IDs: {sorted(list(overlap))}")
